...
API-ключи можно создавать и управлять через веб-интерфейс по адресу `/settings/api-keys`.

**Возможности:**
- **Ограничения по доменам**: Ограничьте API-ключ отправкой только с определенных доменов. Если домены не указаны, ключ может отправлять с любого настроенного домена.
- **Лимиты запросов**: Установите лимиты запросов в минуту и час для каждого ключа.
- **Срок действия**: Опционально установите дату истечения срока действия ключа.
- **Отслеживание активности**: Просмотр времени последнего использования и общего количества отправок.

**Ответы об ошибках:**
| Код | Ошибка | Описание |
|-----|--------|----------|
| `DOMAIN_NOT_ALLOWED` | 403 | API-ключу запрещена отправка с указанного домена |
| `UNAUTHORIZED` | 401 | Неверный или отсутствующий API-ключ |
| `RATE_LIMITED` | 429 | Превышен лимит запросов для этого API-ключа |
...
Поставить в очередь несколько писем одним запросом. Снижает накладные расходы
HTTP и уменьшает contention на write-lock BoltDB при больших рассылках. Все
валидные сообщения ставятся в очередь атомарно в одной транзакции; невалидные
сообщения отмечаются поштучно, не прерывая остальной пакет.
...
Каждый элемент использует ту же схему, что и `POST /api/v1/send`.

Ограничения:
- не более 1000 сообщений в одном запросе;
- размер отдельного сообщения ограничен `smtp.max_message_bytes` (по умолчанию 10 МБ).
...
При ошибках валидации у соответствующего элемента возвращается поле `error`
вместо `id`/`status`:
...
HTTP статус `202 Accepted` возвращается всегда, если сам запрос корректен, даже
если часть элементов отклонена. `400` — при пустом `messages`, `413` — если
превышен лимит в 1000 сообщений.
